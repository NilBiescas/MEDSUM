...
model_id = "/hhome/nlp2_g05/Asho_NLP/src/Trained_models/last_model_LoRA_Finetune_MedicalReports_v2"
...
def compute_metrics(gt: str, pred: str, i: int):
    ...
    os.makedirs(f"/hhome/nlp2_g05/Asho_NLP/src/Plots/{model_id.split('/')[-1]}/freq_histograms", exist_ok=True)
    plt.savefig(f"/hhome/nlp2_g05/Asho_NLP/src/Plots/{model_id.split('/')[-1]}/freq_histograms/histograms_{i}.png")
...
output_file = f"/hhome/nlp2_g05/Asho_NLP/src/gen_sumaries_all_metrics_{model_id.split('/')[-1]}.json"
output_data = []
...
for i in tqdm(range(len(dataset_eval))):
    # Create the input text, it is done in to parts in 
...
os.makedirs(f"/hhome/nlp2_g05/Asho_NLP/src/Plots/{model_id.split('/')[-1]}", exist_ok=True)
for i, key in enumerate(metriques.keys()):
    fig = plt.figure()
    plt.bar(range(len(metriques[key])), np.array(metriques[key]).astype(float))
    plt.title(key)
    plt.savefig(f"/hhome/nlp2_g05/Asho_NLP/src/Plots/{model_id.split('/')[-1]}/{key}.png")

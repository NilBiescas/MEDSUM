...
"""
Code to finetune FLOR-760M to the synthetic dataset generated with LLAMA3-70B in a two-step process, using LoRA on both steps.

First, the model is fine-tuned with CaBreu dataset
Then, the model is fine-tuned with the synthetic dataset generated with LLAMA3-70B
"""

model_name_pretrain = 'Pretrain_FLOR_Articles_summarization_LoRA'
model_name_finetune = 'LoRA_Finetune_MedicalReports_v2_LoRA_pretrained'
...
def preprocess_function(examples):
    ...
    # Concatenate the reports and the summaries inputsidsº
    model_inputs = {"input_ids": [reports_input["input_ids"][i] + summaries_input["input_ids"][i] for i in range(len(reports_input["input_ids"]))]}
    model_inputs["labels"] = model_inputs["input_ids"]
    return model_inputs
